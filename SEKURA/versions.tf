terraform {
  ...
  /* backend "s3" {
    region         = "eu-west-2"
    bucket         = "tf-state-568773044147-indocount"
    key            = "indocount.tfstate"
    dynamodb_table = "tf-state-lock-indocount"
    encrypt        = "true"
  } */
...
/* data "terraform_remote_state" "sekura" {
  backend = "local"

  config = {
    path = "C:/Users/10134215/.aws/terraform.tfstate"
  }
}
 */
