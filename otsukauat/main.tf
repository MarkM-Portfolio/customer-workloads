...
module "custom_sg_rules" {
  ...
  ingress_with_cidr_blocks = [
    ...
    },
    {
      ...
      to_port     = 0
      protocol    = "-1"
      description = "Inbound Traffic from GEN2GRNADSSP01"
      cidr_blocks = "10.245.15.41/32"
    },
     {
      from_port   = 50000
      to_port     = 50000
      protocol    = "-1"
      description = "Inbound Traffic from BOOMI Networks"
      cidr_blocks = "0.0.0.0/0"
    }
  ]
}
...
module "patching" {
  ...
  schedule_london     = "cron(0 00 18 ? * THU#2 *)"
}
