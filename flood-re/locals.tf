locals {
  ...
  sg = {
    ...
    custom_broker      = aws_security_group.floodre_custom_broker_sg.id
    custom_sqluat      = aws_security_group.floodre_custom_sqluat01_sg.id
  }
  ...
  app = {
    GEN2FRBRK01 = {
      ...
      iam_instance_profile   = "SSMInstanceProfile"
      subnet_id              = local.subnet.app_a
      vpc_security_group_ids = [local.sg.baseline_app, local.sg.migration, local.sg.custom_broker]
      root_block_device = [
        {
          encrypted             = true
          volume_type           = "gp3"
    ...
    GEN2FRSQLUAT01 = {
      ...
      vpc_security_group_ids = [local.sg.baseline_data, local.sg.migration, local.sg.custom_sqluat]
      root_block_device = [
        {
          encrypted             = true
          volume_type           = "gp3"
    ...
    # SQL server for DR Test June 2024
    GEN2FRSQL01-DR = {
      ami                    = "ami-033b6932dcf95e273"
      instance_type          = "r5.xlarge"
      iam_instance_profile   = "SSMInstanceProfile"
      subnet_id              = local.subnet.data_b
      vpc_security_group_ids = [local.sg.baseline_data, local.sg.migration]
