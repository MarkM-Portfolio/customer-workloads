locals {
  ...
  sg = {
    ...
    amber_sql          = data.aws_security_group.amber_sql.id
    amber_dmz          = data.aws_security_group.amber_dmz.id
    amber_rds          = aws_security_group.amber-custom-rds-sg.id
  }
  ...
  dc = {
    ...
    DAMB02BRKP01 = {
      ...
      iam_instance_profile   = "SSMInstanceProfile"
      subnet_id              = local.subnet.app_a
      vpc_security_group_ids = [local.sg.migration, local.sg.baseline_app, local.sg.rds_broker, local.sg.amber_rds]
      monitoring             = true
      root_block_device = [
        {
          encrypted             = true
  ...
  ec2 = {
    ...
    GEN2AMBDMZ03 = {
      ...
      iam_instance_profile   = "SSMInstanceProfile"
      subnet_id              = local.subnet.app_a
      vpc_security_group_ids = [local.sg.migration, local.sg.baseline_app, local.sg.amber_dmz]
      monitoring             = true
      root_block_device = [
        {
          encrypted             = true
    ...
    GEN2AMBSQL02 = {
      ...
      iam_instance_profile   = "SSMInstanceProfile"
      subnet_id              = local.subnet.data_a
      vpc_security_group_ids = [local.sg.migration, local.sg.baseline_data, local.sg.amber_sql]
      monitoring             = true
      root_block_device = [
        {
          encrypted             = true
