terraform {
  ...
  backend "s3" {
    region         = "eu-west-2"
    bucket         = "tf-state-817072643255-foresters"
    key            = "foresters.tfstate"
    dynamodb_table = "tf-state-lock-foresters"
    encrypt        = "true"
    role_arn       = "arn:aws:iam::817072643255:role/OrganizationAccountAccessRole"
  }
}
