...
data "aws_security_group" "shared_service_app" {
  vpc_id = data.aws_vpc.this.id
  filter {
    name   = "tag:Name"
    values = ["*-shared-service-app-*"]
  }
}

data "aws_security_group" "infor_os" {
  vpc_id = data.aws_vpc.this.id
  filter {
    name   = "tag:Name"
    values = ["*-infor-*"]
...
# Networking Account Data Sources
...
data "aws_vpc" "networking" {
  filter {
    name   = "tag:Name"
    values = ["vpc-${local.region}-prod-vpc_sec"]
  }
...
# data "aws_subnet" "networking_data_a" {
...
# data "aws_subnet" "networking_data_b" {
#   vpc_id            = data.aws_vpc.networking.id
#   availability_zone = "${local.region}b"
#   filter {
#     name   = "tag:Name"
