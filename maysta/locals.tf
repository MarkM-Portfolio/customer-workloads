locals {
  ...
  sg = {
    ...
    maysta_dmz         = aws_security_group.maysta-custom-dmz-sg.id
  }
  dc = {
  ...
  app = {
    ...
    GEN2MIBRK01 = {
      ...
      iam_instance_profile   = "SSMInstanceProfile"
      subnet_id              = local.subnet.app_a
      vpc_security_group_ids = [local.sg.migration, local.sg.baseline_app, local.sg.rds_broker, aws_security_group.maysta-custom-brk-sg.id]
      root_block_device = [
        {
          encrypted             = true
          volume_type           = "gp3"
    ...
    GEN2MIDMZ01 = {
      ...
      iam_instance_profile   = "SSMInstanceProfile"
      subnet_id              = local.subnet.app_a
      vpc_security_group_ids = [local.sg.migration, local.sg.baseline_app, local.sg.maysta_dmz]
      root_block_device = [
        {
          encrypted             = true
          volume_type           = "gp3"
