terraform {
  ...
  backend "s3" {
    region         = "eu-west-2"
    bucket         = "tf-state-415801384368-tfg-brands"
    key            = "tfg-brands.tfstate"
    dynamodb_table = "tf-state-lock-tfg-brands"
    encrypt        = "true"
#    role_arn       = "arn:aws:iam::415801384368:role/OrganizationAccountAccessRole"
  }
}
